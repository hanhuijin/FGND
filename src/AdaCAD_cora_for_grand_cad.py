...
class AdaCAD(MessagePassing):
    def __init__(self, K, beta, dropout=0.5, edge_index = None, inchannel=7, **kwargs ):
        ...
        self.m2=torch.nn.Parameter(torch.ones(edge_index.shape[1]).cuda(device='cuda:3')*0.01, requires_grad= True)# torch.nn.Linear(edge_amount,edge_amount)
        ...
        self.scale_list=torch.nn.ParameterList([torch.nn.Parameter(torch.tensor([1.0/(self.K)]).cuda(device='cuda:3'), requires_grad= True) for i in range(K)])
        self.lstm=torch.nn.LSTM(input_size=80, hidden_size=80, num_layers=2, batch_first=True).cuda(device='cuda:3')
        self.transformer=torch.nn.Transformer(d_model=80,nhead=8, num_encoder_layers=1, num_decoder_layers=1,dim_feedforward=80,dropout=0.1,activation='relu').cuda(device='cuda:3')
        self.cnn=MyCNN().cuda(device='cuda:3')
        self.m_cat=torch.nn.Linear(160,80).cuda(device='cuda:3')



    def reset_parameters(self):#plus
        self.m1.reset_parameters()
        self.m2=torch.nn.Parameter(torch.ones(self.edge_index.shape[1]).cuda(device='cuda:3')*0.01, requires_grad= True)# torch.nn.Linear(edge_amount,edge_amount)
        self.scale_list=torch.nn.ParameterList([torch.nn.Parameter(torch.tensor([1.0/(self.K)]).cuda(device='cuda:3'), requires_grad= True) for i in range(self.K)])
        self.lstm=torch.nn.LSTM(input_size=80, hidden_size=80, num_layers=2, batch_first=True).cuda(device='cuda:3')
        self.transformer=torch.nn.Transformer(d_model=80,nhead=8, num_encoder_layers=1, num_decoder_layers=1,dim_feedforward=80,dropout=0.1,activation='relu').cuda(device='cuda:3')
        self.cnn=MyCNN().cuda(device='cuda:3')
        self.m_cat=torch.nn.Linear(160,80).cuda(device='cuda:3')
        # self.bite_of_conti=torch.nn.Parameter(torch.ones(1),requires_grad=True).cuda(device='cuda:3')
    ...
    def forward(self, x,   is_debug=False):
        ...
        with torch.no_grad():
            ...
            deg[deg==0] = 1
            cont_i = (sum_pipj) / deg#torch.sqrt(deg) 
