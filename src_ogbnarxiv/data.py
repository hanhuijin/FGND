...
def get_dataset(opt: dict, data_dir, use_lcc: bool = False) -> InMemoryDataset:
  ...
  elif ds in ['ogbn-arxiv','ogbn-products','ogbn-proteins','ogbn-papers100M']:
...
def set_train_val_test_split(
  ...
  return data
def set_train_val_test_split_for_arxiv(
        seed: int,
        data: Data,
        num_development: int = 1500,
        num_per_class: int = 20) -> Data:
  ...
  val_idx = []
  for c in range(data.y.max() + 1):
      class_idx = idx[np.where(data.y[idx].cpu() == c)[0]]
      if len(class_idx)<2:
        idx=np.setdiff1d(idx,class_idx)
        continue
      print(c,len(class_idx))
      train_temp=rnd_state.choice(class_idx, int(0.1*len(class_idx)+1), replace=False)
      # for i in range(11//len(train_temp)):
      train_idx.extend(train_temp)#num_per_class
      class_idx_deltrain=np.setdiff1d(class_idx,train_temp)
      val_temp=rnd_state.choice(class_idx_deltrain, int(0.1*len(class_idx)+1), replace=False)
      # for i in range(11//len(train_temp)):
      val_idx.extend(train_temp)#num_per_class
  # idx_deltrain=np.setdiff1d(idx,train_idx)
  # val_idx = rnd_state.choice(idx_deltrain, num_development-len(train_idx), replace=False)



  development_idx = []
  development_idx.extend(val_idx)
  development_idx.extend(train_idx)
  idx_deldevelopment_idx=np.setdiff1d(idx,development_idx)
  test_idx = idx_deldevelopment_idx
  # test_idx = rnd_state.choice(idx_deldevelopment_idx, 10000, replace=False)#[i for i in np.arange(num_nodes) if i not in development_idx]
