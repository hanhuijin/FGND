...
from data import get_dataset, set_train_val_test_split,set_train_val_test_split_for_arxiv
...
def plot_embeddings(embeddings, label_file):
    ...
    plt.legend()
    plt.show()
    plt.savefig('FlexLCRGNN_Pubmed14.png')
...
def train(model, optimizer, data, pos_encoding=None):
  ...
  optimizer.zero_grad()
  feat = data.x
  if model.opt['use_labels']:#plus module 为了多gpu训练
  ...
  if model.opt['dataset'] == 'ogbn-arxiv':#plus module 为了多gpu训练
    lf = torch.nn.functional.nll_loss
    # test=len(data.y.squeeze(1)[data.train_mask])
    loss = lf(out.log_softmax(dim=-1)[data.train_mask], data.y.squeeze(1)[data.train_mask])
  else:
    lf = torch.nn.CrossEntropyLoss()
    loss = lf(out[data.train_mask], data.y.squeeze()[data.train_mask])
  if model.odeblock.nreg > 0:  # add regularisation - slower for small data, but faster and better performance for large data#plus module 为了多gpu训练
...
@torch.no_grad()
def _test(model, data, pos_encoding=None, opt=None,calculate_or_not=False):  # opt required for runtime polymorphism
  model.eval()
  feat = data.x
  if model.opt['use_labels']:
  ...
  #plus计算邻接节点中同类节点的比率----------------------------------------------
  if calculate_or_not==True:
    model.calculate_same_class_ratio(feat)
#--------------------------------------------------------------------------------------------



  for _, mask in data('train_mask', 'val_mask', 'test_mask'):
    # test=torch.tensor(range(len(mask)))
    # test=test[mask]
...
@torch.no_grad()
def _test_OGB(model, data, pos_encoding, opt,calculate_or_not=False):
...
def main(cmd_opt):
  ...
  if opt['rewire_KNN'] or opt['fa_layer']:
    ...
    # model=GNNEarly(opt, dataset, device).to(device)#test for compare 
    model = GNN(opt, dataset, device).to(device) if opt["no_early"] else GNNEarly(opt, dataset, device).to(device)#delete for compare
  ...
  this_test = _test_OGB if opt['dataset'] == 'ogbn-arxiv' else _test
  
  # 多次训练求均值
  val_losses, accs, durations = [], [], []
  for run_num in range(20):
    best_time = best_epoch = train_acc = val_acc = test_acc = 0
    model.reset_parameters()#初始化模型参数
    # dataset.data.to('cpu')
    # if opt['rewire_KNN'] or opt['fa_layer']:
    #   model = GNN_KNN(opt, dataset, device).to(device) if opt["no_early"] else GNNKNNEarly(opt, dataset, device).to(device)
    # else:
    ...
    # 重新随机划分训练集
    # if not opt['planetoid_split'] and opt['dataset'] in ['Cora','Citeseer','Pubmed']:
    # if not opt['planetoid_split'] and opt['dataset'] in ['Cora', 'Citeseer', 'Pubmed', 'Computers', 'Photo']:
    ...
    if (opt["dataset"] == "ogbn-arxiv") :
      ...
      data = Data(
      x=dataset.data.x.cuda(),
      edge_index=ei,
      y=dataset.data.y.cuda(),
      train_mask=split_idx['train'].cuda(),
      test_mask=split_idx['test'].cuda(),
      val_mask=split_idx['valid'].cuda())
      dataset.data = data
    elif (opt['dataset']=='cornell')or(opt['dataset']=='texas')or(opt['dataset']=='squirrel')or(opt['dataset']=='chameleon') :
      dataset.data = set_train_val_test_split_for_arxiv(np.random.randint(0, 1000), dataset.data, num_development=0.2*dataset.data.num_nodes  ,num_per_class=0.2*dataset.data.num_nodes)#if (opt["dataset"] == "cornell")or(opt['dataset']=='texas')    else 1500
    ...
    for epoch in range(1, 100):#opt['epoch']
      ...
      if not opt['no_early'] and model.odeblock.test_integrator.solver.best_val > val_acc:
        ...
        # model.odeblock.t=torch.tensor([0, model.odeblock.test_integrator.solver.best_time]).to(device)#plus,尝试一下在训练的过程中优化train中的odeint的t
    ...
    #plus just 计算邻接节点中同类节点的比率
    # tmp_train_acc, tmp_val_acc, tmp_test_acc = this_test(model, data, pos_encoding, opt,calculate_or_not=True)
    #-----------------------------------------------------------------------------------------------------------------
    # emb=model(data.x)
    # plot_embeddings(emb.to('cpu').detach().numpy(),data.y.to('cpu').detach().numpy())
    accs.append(test_acc)
    durations.append(best_time)
    print('Run{:d},best val accuracy {:03f} with test accuracy {:03f} at epoch {:d} and best time {:03f},-mean of testacc{:.3f}'.format(run_num+1,val_acc, test_acc,
                                                                                                      best_epoch,
                                                                                                      best_time,
                                                                                                      torch.tensor(accs).mean().item()))
    
  acc, duration = torch.tensor(accs),torch.tensor(durations)

  print('============================= Total =============================')
  print('Test Accuracy: {:.3f} ± {:.3f}, Time(ms): {:.2f}'.
          format(
                 acc.mean().item(),
                 acc.std().item(),
                 duration.mean().item()))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--dataset', type=str, default='ogbn-arxiv',
                      help='Cora, Citeseer, Pubmed, Computers, Photo, CoauthorCS, ogbn-arxiv,CoauthorPHYSICS')
  ...
  # ODE args
  parser.add_argument('--time', type=float, default=1, help='End time of ODE integrator.')
  parser.add_argument('--augment', action='store_true',
                      help='double the length of the feature vector by appending zeros to stabilist ODE learning')
  parser.add_argument('--method', type=str,default='dopri5', help="set the numerical solver: dopri5, euler, rk4, midpoint")#dopri5
  ...
  parser.add_argument("--no_early",default=False, action="store_true",
