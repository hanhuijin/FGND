...
for epoch in range(args.epochs):
    ...
    (train_adj, train_fea) = sampler.randomedge_sampler(percent=args.sampling_percent, normalization=args.normalization,
                                                        cuda=device)#args.cuda
    if args.mixmode:
        train_adj = train_adj.cuda(device)
    ...
    if False:
        ...
        (val_adj, val_fea) = sampler.get_test_set(normalization=args.normalization, cuda=device)#args.cuda
        if args.mixmode:
            val_adj = val_adj.cuda(device)
        # print(train_adj.device, train_fea.device, input_idx_train.device, val_adj.device, val_fea.device)
        outputs = train(epoch, train_adj, train_fea, input_idx_train, val_adj, val_fea)

    if args.debug and False and epoch % 1 == 0:
