...
class Sampler:
    ...
    def _preprocess_adj(self, normalization, adj, cuda):
        ...
        if cuda:
            r_adj = r_adj.cuda(device)
        return r_adj

    def _preprocess_fea(self, fea, cuda):
        if cuda:
            return fea.cuda(device)
        else:
            return fea
    ...
    def get_label_and_idxes(self, cuda):
        ...
        """
        if cuda:
            return self.labels_torch.cuda(device), self.idx_train_torch.cuda(device), self.idx_val_torch.cuda(device), self.idx_test_torch.cuda(device)
